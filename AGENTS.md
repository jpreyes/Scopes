...
- **Quote number**: una propuesta nace con N.º provisorio `PROV-XXXXXX` (`provisionalNumber()`) y lo conserva en todas sus modificaciones; el correlativo `CT-PS-{NNN}-{YYYY}` (mayor existente + 1) lo asigna **el servidor** (`pocketbase/pb_hooks/lib/quotes.js`) cuando el estado pasa a `aprobada` o posterior, y desde ahí es inmutable. Solo se renumeran los `PROV-…` (los `DEMO-…` de los datos de ejemplo y los CT-PS viejos no). `quoteNumber` tiene índice único. Ya no existe `presto_counter` ni `generateQuoteNumber()`: el contador por navegador hacía que cada «Guardar» creara una copia con otro número y que dos personas sacaran el mismo número y se pisaran.
- **Currency**: `$` → 0 decimals with `.` thousands sep; `UF`/`US$`/`€` → 2 decimals.
- **Persistence**: la propuesta abierta se guarda **por id** (`state.quoteId`), nunca por N.º: `persistBudget(okMsg)` encola los guardados (`saveChain`; el segundo espera el id que trae el primero), hace PATCH/POST y muestra el aviso **cuando el servidor confirma** (antes decía «Guardado ✓» aunque fallara). Antes de un PATCH compara el `updatedAt` del servidor con el que cargó (`knownUpdatedAt`) y, si otra persona guardó entretanto, pide confirmación. `localStorage` (`presto_{quoteNumber}`, `presto_list`) es solo respaldo; `migrateLocalToPB()` sube únicamente lo guardado sin conexión (`_pending`). `loadBudgetByNum()` = `applyIdentity()` (N.º, estado, votos, autoría) + `applyContent()`, que **reemplaza todo** el contenido: costeo, grupos y Gantt incluidos, conservando los ids/`_key` guardados y subiendo el contador `uid` por encima de ellos (`bumpUid`/`normalizeIds`).
- **Versiones**: colección `quote_versions`, la escribe **solo** el hook en cada create/update de `quotes` que cambie algo (firma sha256 sin `updatedBy`/`updatedAt`), con las últimas 40 por propuesta. La UI (botón «Versiones» del Documento) lista y `restoreVersion()` aplica solo el contenido (`applyContent`) y guarda: el N.º, el estado y las aprobaciones no retroceden.
...
- **resetBudget()** clears proposal form (nuevo N.º provisorio, `quoteId` vacío), preserves company info.
...
- **Trazabilidad**: `quotes` y `proyectos` llevan `createdBy`/`createdAt`/`updatedBy`/`updatedAt` (text ISO). En `quotes` los fija **el servidor** (`pb_hooks/lib/quotes.js`): `createdBy`/`createdAt` al crear y nunca más; `updatedBy` desde la sesión en cada guardado. El cliente los manda solo al crear. (Antes el cliente los reescribía y `createdBy` terminaba siendo quien guardara desde un navegador sin copia local — "creado" y "modificado" salían siempre iguales.) `proyectos` sigue con `withTrace()` en el cliente. Historial muestra "Creado por" y "Modificado" (`fmtStamp` dd/mm hh:mm).
- **Aprobación interna de propuestas**: campo `aprobaciones` (JSON array `{by, at}`) en la colección `quotes` (y en `collectData`/`loadBudgetByNum`/`resetBudget`). `aprobarPropuesta()` (store) exige que el aprobador sea distinto del creador (`createdBy`) y no repita voto; solo aplica en estado `en_revision`, y con la 2ª aprobación auto-transiciona a `aprobada`. `aprobacionInfo` computed → `{ lista, count (usuarios distintos), listaParaEnviar (count>=2) }`. El estado "Enviada" se bloquea con <2 aprobaciones: el select de estado en PropuestaTab usa `:value` + `@change` con `onStatusChange()` (NO `v-model` — revertir el estado reactivo dentro de un watch dejaba el DOM desincronizado con vModelSelect). `saveBudget()` = `persistBudget()`; las transiciones pasan su aviso como `persistBudget('…')`, y `persistBudget()` adopta del servidor `quoteId`, `quoteNumber`, `createdBy` y `createdAt`. OJO: PB descarta silenciosamente campos que no existen en el esquema — al agregar un campo al state hay que agregarlo a la colección (migración).
...
- **`dedupeQuotes()` ya no existe**: borraba en silencio, en cada login, toda propuesta que compartiera N.º con otra, y dos propuestas distintas lo compartían justamente por el contador por navegador. Hoy el N.º es único en el servidor.
- **`aprobarPropuesta()`** relee el registro antes de votar: si solo cambiaron votos los une; si cambió el contenido, recarga y pide revisar de nuevo (votar sobre la copia vieja borraba el voto del otro).
